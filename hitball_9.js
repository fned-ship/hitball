        ...
        class Orientation{
            ...
            orientation(){
                if(screen.orientation.type =="portrait-primary" || screen.orientation.type =="portrait-secondary"  ){
                    ...
                    //console.log(cvs.height+"..."+windowHeight)
...
function levelUp(){
    ...
    if(isLevelDone){
        ...
        if(LEVEL==0){
            ...
            //console.log("***creating"+Math.floor((cvs.width)/(dmbr+dmbr/4))+"bricks(columns)")
            setLevel("0");
            localStorage.setItem("level","0");
            setBorn("10");
            ...
            createBrickswall(1,1,cvs.width/3,2*cvs.width/3,dmbr,(Math.floor((cvs.height - PADDLE_MARGIN_BOTTOM - PADDLE_HEIGHT-50)/(dmbr+dmbr/4))-8)*(dmbr+dmbr/4)+50+4+dmbr+5+dmbr,cvs.width/3,4,false);
...
//enter
function enter1(){
    if(document.fullscreenElement==null && vrb>4 && vrb1>=3 && GAME_OVER!=true ){
        //console.log("*******exit*******");
...
function update(){
    ...
    //if(vrb<7){
      //  console.log("***about to inter levelUp***");
    //}
